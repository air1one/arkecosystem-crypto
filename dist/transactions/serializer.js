...
class Serializer {
    static getBytes(transaction, options = {}) {
        ...
        if (options.acceptLegacyVersion || options.disableVersionCheck || utils_2.isSupportedTransactionVersion(version)) {
            if (version === 1) {
    ...
    static getBytesV1(transaction, options = {}) {
        let assetSize = 0;
        let assetBytes;
        if (transaction.type === enums_1.TransactionType.SecondSignature && transaction.asset) {
        ...
        if (transaction.type === enums_1.TransactionType.DelegateRegistration &&
            transaction.asset &&
            transaction.asset.delegate) {
            assetBytes = Buffer.from(transaction.asset.delegate.username, "utf8");
            assetSize = assetBytes.length;
        }
        if (transaction.type === enums_1.TransactionType.Vote && transaction.asset && transaction.asset.votes) {
            assetBytes = Buffer.from(transaction.asset.votes.join(""), "utf8");
            assetSize = assetBytes.length;
        }
        if (transaction.type === enums_1.TransactionType.MultiSignature &&
            transaction.asset &&
    ...
    static serializeSignatures(transaction, buffer, options = {}) {
        ...
        if (transaction.signatures) {
            if (transaction.version === 1 && utils_1.isException(transaction)) {
